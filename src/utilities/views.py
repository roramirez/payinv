...
class ObjectListView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Attempt to redirect automatically if the search query returns a
        # single result
        if self.redirect_on_single_result and self.queryset.count() == 1 and request.GET:
            try:
                return HttpResponseRedirect(
                    self.queryset[0].get_absolute_url())
        ...
        table.model = model
        if 'pk' in table.base_columns and any(
                [request.user.has_perm(perm) for perm in self.edit_permissions]):
            table.base_columns['pk'].visible = True
        RequestConfig(
            request, paginate={
                'klass': EnhancedPaginator}).configure(table)
        ...
        if kwargs:
            ...
            if callable(self.form_class):
                form = self.form_class(
                    initial={
                        k: request.GET.get(k) for k in self.fields_initial})
            else:
                form = self.form_class
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if hasattr(obj, 'get_absolute_url'):
                msg = u'{} <a href="{}">{}</a>'.format(
                    msg, obj.get_absolute_url(), obj)
            else:
...
class BulkEditView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if posted_redirect_url and is_safe_url(
                url=posted_redirect_url, host=request.get_host()):
        ...
        if request.POST.get('_all'):
            pk_list = [int(pk)
                       for pk in request.POST.get('pk_all').split(',') if pk]
        else:
        ...
        if '_apply' in request.POST:
            ...
            if form.is_valid():
                ...
                # Update objects
                updated_count = self.update_objects(
                    pk_list, form, standard_fields)

                # Update custom fields for objects
                if custom_fields:
                    objs_updated = self.update_custom_fields(
                        pk_list, form, custom_fields)
                    if objs_updated and not updated_count:
                        updated_count = objs_updated

                if updated_count:
                    msg = u'Updated {} {}'.format(
    ...
    def update_objects(self, pk_list, form, fields):
        ...
        for name in fields:
            ...
            if isinstance(
                    form.fields[name], TypedChoiceField) and form.cleaned_data[name] == 0:
                fields_to_update[name] = None
            elif form.cleaned_data[name]:
                fields_to_update[name] = form.cleaned_data[name]

        return self.cls.objects.filter(
            pk__in=pk_list).update(**fields_to_update)
    ...
    def update_custom_fields(self, pk_list, form, fields):
        ...
        for name in fields:
            if form.cleaned_data[name] not in [None, u'']:
                ...
                # Gather any pre-existing CustomFieldValues for the objects
                # being edited.
                existing_cfvs = CustomFieldValue.objects.filter(
                    field=field, obj_type=obj_type, obj_id__in=pk_list)

                # Determine which objects have an existing CFV to update and
                # which need a new CFV created.
...
class BulkDeleteView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Are we deleting *all* objects in the queryset or just a selected
        # subset?
        if request.POST.get('_all'):
